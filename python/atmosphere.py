...
if not os.path.isfile(directory+'atmosphere.in'):
    sys.exit('The atmosphere.in file does not exist in '+atmosphere+'!')
...
if os.path.isfile(directory+'pressureTemperature.dat'):

    # Mean molecular mass [kg/mol]
    mmw = float(parser.get('composition', 'molweight')) * 1.e-3 # [g/mol] -> [kg/mol]
    # Surface gravity
    log_g = float(parser.get('composition', 'log_g'))
    gravity = 1e-2*10.**log_g # [m s-2]

    # Pressure temperature profile
    pressure, temperature = np.loadtxt(directory+'pressureTemperature.dat', unpack=True)
    # [bar] -> [Pa]
    pressure *= 1.e5
    # Reverse order
    pressure = pressure[::-1]
...
if gas:
    ...
    n_wavelength = np.size(opacity,1)
    wavelengths = opacity[0]

    opacityGas = np.zeros((np.size(densityGas),4,n_wavelength))
    scatterGas = np.zeros((np.size(densityGas),180,16,n_wavelength))
...
if opacityNumber > 0:
    ...
    opacityOther = np.zeros((opacityNumber,4,n_wavelength))
    scatterOther = np.zeros((opacityNumber,180,16,n_wavelength))
...
opacityScattering = np.zeros((n_wavelength, nphi, ntheta-1, nr-1))
opacityAbsorption = np.zeros((n_wavelength, nphi, ntheta-1, nr-1))
scatter = np.zeros((180, 16, n_wavelength, nphi, ntheta-1, nr-1))
...
# Write atmospheric structure

if os.path.isfile(directory+'pressureTemperature.dat'):

    z = np.array(zip(pressure*1.e-5, temperature, densityGas*1.e-3, scaleHeight*1.e-3, radialGas*1.e-3))
    f = open(directory+'atmosphere.dat','w')
    f.write('# Pressure [bar] - Temperature [K] - Gas density [g/cm3] - Scale Height [km] - Altitude [km] \n\n')
    np.savetxt(f,z)
    f.close()
...
if ring:
    ...
    ring_opacity_scat = np.zeros((n_wavelength,nphi,ntheta-1,2))
    ring_opacity_abs = np.zeros((n_wavelength,nphi,ntheta-1,2))
    ring_scatter = np.zeros((180,16,n_wavelength,nphi,ntheta-1,2))
    
    for m in range(n_wavelength):
    
        oScat = float(aa[1])*opacityOther[int(aa[0])-1,3,m]
        oAbs = float(aa[1])*opacityOther[int(aa[0])-1,2,m]
...
hdunew.writeto(directory+'atmosphere.fits', clobber=True)
hdunew.close()